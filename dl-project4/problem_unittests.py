...
def test_model_inputs(model_inputs):
    with tf.Graph().as_default():
        ...
        with tf.control_dependencies([tf.assert_rank(input_data, 2, message='Input data has wrong rank'),
...
def test_decoding_layer(decoding_layer):
    ...
    with tf.Graph().as_default():
        ...
        assert train_decoder_output.rnn_output.get_shape().as_list() == [batch_size, None, vocab_size], \
            'Wrong shape returned.  Found {}, expected {}'.format(train_decoder_output.rnn_output.get_shape(), [batch_size, None, vocab_size])
        assert infer_logits_output.sample_id.get_shape().as_list() == [batch_size, None], \
             'Wrong shape returned.  Found {}, expected {}'.format(infer_logits_output.sample_id.get_shape(), [batch_size, None])
